...
fit <- stan(file = 'Myc_model_trees_simplified.stan', data = calib_data,  chains = 4, iter = iterations, cores=4, warmup = iterations-chain_length)
########################################################################################################
...
parms_names<-c(expression(paste(mu,"1")),
               ...
               expression(paste(mu,"4")),
               expression(paste(mu,"5")),
               expression(paste(mu,"6")),"a", expression(paste(P[0]," control/apatite")),
...
parms_list<-c("mu1","mu2","mu3","mu4", "mu5", "mu6",  "a", "p1", "p2", "p3", "p4", "p5", "p6")


###plot the chains for diagnostic purposes (should be mixing well, if you see some straight line it's really bad)
png("random_walks_variablemu_with_treatments.png", width=4000, height=2500, res=300)
par(mfrow=c(3,4))
...
plot(posteriors_cp[,1,"a"], type="l", col=chain_palette[1], ylab=parms_names[6], xlab="MCMC index")
for(i in 2:4){lines(posteriors_cp[,i,"a"], col=chain_palette[i])}
...
png("parms_distributions_variablemu_with_treatments.png", width=4000, height=1200, res=350)
par(mfrow=c(1,3))
...
d_mu5<-density(posteriors_cp[,,parms_list[5]]) #kernel density estimation
d_mu6<-density(posteriors_cp[,,parms_list[6]]) #kernel density estimation
...
polygon(d_mu1, col=times_palette_alpha[1], border=times_palette_alpha[1]) #fill the area under the curve with a semitransparent color
text(mlv(d_mu1$x, method = "Parzen"),max(d_mu1$y)*1.01, class_names[1], col=parms_palette[1], cex=0.8) #add a text on top of the distribution
polygon(d_mu2, col=times_palette_alpha[2], border=times_palette_alpha[2]) #fill the area under the curve with a semitransparent color
text(mlv(d_mu2$x, method = "Parzen"), max(d_mu2$y)*1.01, class_names[2], col=parms_palette[2], cex=0.8) #add a text on top of the distribution
polygon(d_mu3, col=times_palette_alpha[3], border=times_palette_alpha[3]) #fill the area under the curve with a semitransparent color
text(mlv(d_mu3$x, method = "Parzen")*0.8, max(d_mu3$y)*1.01, class_names[3], col=parms_palette[3], cex=0.8) #add a text on top of the distribution
polygon(d_mu4, col=times_palette_alpha[4], border=times_palette_alpha[4]) #fill the area under the curve with a semitransparent color
text(mlv(d_mu4$x, method = "Parzen")*0.8, max(d_mu4$y)*1.01, class_names[4], col=parms_palette[4], cex=0.8) #add a text on top of the distribution
polygon(d_mu5, col=times_palette_alpha[5], border=times_palette_alpha[5]) #fill the area under the curve with a semitransparent color
text(mlv(d_mu5$x, method = "Parzen")*0.8, max(d_mu5$y)*1.01, class_names[5], col=parms_palette[4], cex=0.8) #add a text on top of the distribution
polygon(d_mu6, col=times_palette_alpha[6], border=times_palette_alpha[6]) #fill the area under the curve with a semitransparent color
text(mlv(d_mu6$x, method = "Parzen")*0.8, max(d_mu6$y)*1.01, class_names[6], col=parms_palette[4], cex=0.8) #add a text on top of the distribution
legend("topright","(a)", bty="n")

d_z<-density(posteriors_cp[,,parms_list[7]]) #kernel density estimation
plot(d_z, type="l", col=parms_palette[7], ylab="Probability density", xlab=parms_names[7], main="")
polygon(d_z, col=parms_palette_alpha[7], border=parms_palette[7]) #fill the area under the curve with a semitransparent color
legend("topright","(b)", bty="n")


#one plot for all the p values
d_p1<-density(posteriors_cp[,,parms_list[8]]) #kernel density estimation
d_p2<-density(posteriors_cp[,,parms_list[9]]) #kernel density estimation
d_p3<-density(posteriors_cp[,,parms_list[10]]) #kernel density estimation
d_p4<-density(posteriors_cp[,,parms_list[11]]) #kernel density estimation
d_p5<-density(posteriors_cp[,,parms_list[12]]) #kernel density estimation
d_p6<-density(posteriors_cp[,,parms_list[13]]) #kernel density estimation
...
#plotting the rates
png("Boxplot_rates_variablemu_with_treatments.png", width=2500, height=2500, res=350)
par(mar=c(10,5,2,2))
...
png("Boxplot_rates_times_variablemu_with_treatments.png", width=2500, height=2500, res=350)
par(mar=c(10,5,2,2))
...
cv_simulated_p6<-mat.or.vec(dim(resampled_posteriors)[1], length(calib_data$t_6))
  ...
  for( i in 1: dim(resampled_posteriors)[1]){
   ...
   #calculate the biomass
    ...
    mu5=resampled_posteriors$mu5[i]
    mu6=resampled_posteriors$mu6[i]
    om=(2*pi)/365.25
...
plot(calib_data$biom1, means, ylim=plotrange, xlim=plotrange, col=times_palette[1], main=class_names[1], pch=as.numeric(as.factor(round(calib_data$t_1/10)*10)), ylab=expression(paste("Predicted, Biomass, g m"^"-2")), xlab=expression(paste("Measured, Biomass, g m"^"-2")))
arrows(calib_data$biom1, means-sds, calib_data$biom1, means+sds, length=0.03, angle=90, code=3, col=times_palette_alpha[1])
legend("topright","(a)", bty="n")
legend("topleft", levels(as.factor(round(calib_data$t_1/10)*10)), pch=1:5,cex=1.2, bty="n")


means<-colMeans(biomass_simulated_p2, na.rm = T)
sds<-colSdApply(biomass_simulated_p2)
plotrange=c(0, max(c(means,calib_data$biom2)))
plot(calib_data$biom2, means, ylim=plotrange, xlim=plotrange, col=times_palette[2], pch=as.numeric(as.factor(round(calib_data$t_2/10)*10)), main=class_names[2], ylab=expression(paste("Predicted, Biomass, g m"^"-2")), xlab=expression(paste("Measured, Biomass, g m"^"-2")))
...
plot(calib_data$biom3, means, ylim=plotrange, xlim=plotrange, col=times_palette[3], pch=as.numeric(as.factor(round(calib_data$t_2/10)*10)), main=class_names[3], ylab=expression(paste("Predicted, Biomass, g m"^"-2")), xlab=expression(paste("Measured, Biomass, g m"^"-2")))
...
plot(calib_data$biom4, means, ylim=plotrange, xlim=plotrange, col=times_palette[4], pch=as.numeric(as.factor(round(calib_data$t_2/10)*10)), main=class_names[4], ylab=expression(paste("Predicted, Biomass, g m"^"-2")), xlab=expression(paste("Measured, Biomass, g m"^"-2")))
...
plot(calib_data$biom5, means, ylim=plotrange, xlim=plotrange, col=times_palette[5], pch=as.numeric(as.factor(round(calib_data$t_3/10)*10)), main=class_names[5], ylab=expression(paste("Predicted, Biomass, g m"^"-2")), xlab=expression(paste("Measured, Biomass, g m"^"-2")))
...
plot(calib_data$biom6, means, ylim=plotrange, xlim=plotrange, col=times_palette[6], pch=as.numeric(as.factor(round(calib_data$t_2/10)*10)), main=class_names[6], ylab=expression(paste("Predicted, Biomass, g m"^"-2")), xlab=expression(paste("Measured, Biomass, g m"^"-2")))
...
png("Scatter_plot_MAE_vs_meas_means_variablemu_with_treatments.png", width=4000, height=2500, res=390)
par(mfrow=c(2,3), mar=c(5,5,2,2))
...
png("Boxplot_RMSE_variablemu_with_treatments.png", width=2000, height=2500, res=350)
par(mar=c(10,5,2,2))
...
res1<-as.data.frame(t(rbind(calib_data$t0_1+182, calib_data$t_1+182,res_simulated_p1)))
res2<-as.data.frame(t(rbind(calib_data$t0_2+182, calib_data$t_2+182,res_simulated_p2)))
res3<-as.data.frame(t(rbind(calib_data$t0_3+182, calib_data$t_3+182,res_simulated_p3)))
res4<-as.data.frame(t(rbind(calib_data$t0_4+182, calib_data$t_4+182,res_simulated_p4)))
res5<-as.data.frame(t(rbind(calib_data$t0_5+182, calib_data$t_5+182,res_simulated_p5)))
res6<-as.data.frame(t(rbind(calib_data$t0_6+182, calib_data$t_6+182,res_simulated_p6)))
...
sim1<-as.data.frame(t(rbind(calib_data$t0_1+182, calib_data$t_1+182,biomass_simulated_p1)))
sim2<-as.data.frame(t(rbind(calib_data$t0_2+182, calib_data$t_2+182,biomass_simulated_p2)))
sim3<-as.data.frame(t(rbind(calib_data$t0_3+182, calib_data$t_3+182,biomass_simulated_p3)))
sim4<-as.data.frame(t(rbind(calib_data$t0_4+182, calib_data$t_4+182,biomass_simulated_p4)))
sim5<-as.data.frame(t(rbind(calib_data$t0_5+182, calib_data$t_5+182,biomass_simulated_p5)))
sim6<-as.data.frame(t(rbind(calib_data$t0_6+182, calib_data$t_6+182,biomass_simulated_p6)))
...
dataframe_MAE_p1<-as.data.frame(t(rbind(calib_data$t_1,res_simulated_p1)))
dataframe_MAE_p2<-as.data.frame(t(rbind(calib_data$t_2,res_simulated_p2)))
dataframe_MAE_p3<-as.data.frame(t(rbind(calib_data$t_3,res_simulated_p3)))
dataframe_MAE_p4<-as.data.frame(t(rbind(calib_data$t_4,res_simulated_p4)))
dataframe_MAE_p5<-as.data.frame(t(rbind(calib_data$t_5,res_simulated_p5)))
dataframe_MAE_p6<-as.data.frame(t(rbind(calib_data$t_6,res_simulated_p6)))
...
par(mfrow=c(2,3), mar=c(5,5,2,2))
boxplot(t(dataframe_MAE_p1_mean), names=unique(calib_data$t_1+182), ylab=expression(paste("MAE, g m"^"-2")), xlab="Julian day end", col=times_palette_alpha[1], main=class_names[1], ylim=MAE_range)
boxplot(t(dataframe_MAE_p2_mean), names=unique(calib_data$t_2+182), ylab=expression(paste("MAE, g m"^"-2")), xlab="Julian day end", col=times_palette_alpha[2], main=class_names[2], ylim=MAE_range)
boxplot(t(dataframe_MAE_p3_mean), names=unique(calib_data$t_3+182), ylab=expression(paste("MAE, g m"^"-2")), xlab="Julian day end", col=times_palette_alpha[3], main=class_names[3], ylim=MAE_range)
boxplot(t(dataframe_MAE_p4_mean), names=unique(calib_data$t_4+182), ylab=expression(paste("MAE, g m"^"-2")), xlab="Julian day end", col=times_palette_alpha[4], main=class_names[4], ylim=MAE_range)
boxplot(t(dataframe_MAE_p5_mean), names=unique(calib_data$t_5+182), ylab=expression(paste("MAE, g m"^"-2")), xlab="Julian day end", col=times_palette_alpha[5], main=class_names[5], ylim=MAE_range)
boxplot(t(dataframe_MAE_p6_mean), names=unique(calib_data$t_6+182), ylab=expression(paste("MAE, g m"^"-2")), xlab="Julian day end", col=times_palette_alpha[6], main=class_names[6], ylim=MAE_range)
dev.off()


# plotting mean absolute error per time point
dataframe_MAE_p1<-as.data.frame(t(rbind(calib_data$t0_1,res_simulated_p1)))
dataframe_MAE_p2<-as.data.frame(t(rbind(calib_data$t0_2,res_simulated_p2)))
dataframe_MAE_p3<-as.data.frame(t(rbind(calib_data$t0_3,res_simulated_p3)))
dataframe_MAE_p4<-as.data.frame(t(rbind(calib_data$t0_4,res_simulated_p4)))
dataframe_MAE_p5<-as.data.frame(t(rbind(calib_data$t0_5,res_simulated_p5)))
dataframe_MAE_p6<-as.data.frame(t(rbind(calib_data$t0_6,res_simulated_p6)))
...
dataframe_CV_p1<-as.data.frame(t(rbind(calib_data$t_1,cv_simulated_p1)))
dataframe_CV_p2<-as.data.frame(t(rbind(calib_data$t_2,cv_simulated_p2)))
dataframe_CV_p3<-as.data.frame(t(rbind(calib_data$t_3,cv_simulated_p3)))
dataframe_CV_p4<-as.data.frame(t(rbind(calib_data$t_4,cv_simulated_p4)))
dataframe_CV_p5<-as.data.frame(t(rbind(calib_data$t_5,cv_simulated_p5)))
dataframe_CV_p6<-as.data.frame(t(rbind(calib_data$t_6,cv_simulated_p6)))
...
par(mfrow=c(2,3), mar=c(5,5,2,2))
boxplot(t(dataframe_CV_p1_mean)*100, names=unique(calib_data$t_1+182), ylab="Coefficient of variation, %", xlab="Julian day", col=times_palette_alpha[1], main=class_names[1], ylim=CV_range)
boxplot(t(dataframe_CV_p2_mean)*100, names=unique(calib_data$t_2+182), ylab="Coefficient of variation, %", xlab="Julian day", col=times_palette_alpha[2], main=class_names[2], ylim=CV_range)
boxplot(t(dataframe_CV_p3_mean)*100, names=unique(calib_data$t_3+182), ylab="Coefficient of variation, %", xlab="Julian day", col=times_palette_alpha[3], main=class_names[3], ylim=CV_range)
boxplot(t(dataframe_CV_p4_mean)*100, names=unique(calib_data$t_4+182), ylab="Coefficient of variation, %", xlab="Julian day", col=times_palette_alpha[4], main=class_names[4], ylim=CV_range)
boxplot(t(dataframe_CV_p5_mean)*100, names=unique(calib_data$t_5+182), ylab="Coefficient of variation, %", xlab="Julian day", col=times_palette_alpha[5], main=class_names[5], ylim=CV_range)
boxplot(t(dataframe_CV_p6_mean)*100, names=unique(calib_data$t_6+182), ylab="Coefficient of variation, %", xlab="Julian day", col=times_palette_alpha[6], main=class_names[6], ylim=CV_range)
dev.off()
