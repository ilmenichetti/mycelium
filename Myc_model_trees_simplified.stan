...
model { //this block runs the model and the MCMC sampling
  ...
  for (i in 1:N){ // loop for each class
        ...
        biomass1[i]= (p1)/mu1*(1-exp(-mu1*(time1[i]-start1[i])))+
                    ((a*p1)/mu1)*(cos(om)*time1[i]-exp(-mu1*(time1[i]-start1[i]))*cos(om)*start1[i])-
                     ((a*p1)/(om*mu1))*(1/(1+(1/(om^2*mu1^2))))*((1/mu1)*(sin(om)*time1[i]-exp(-mu1*(time1[i]-start1[i]))*sin(om)*start1[i])+
        ...
        biomass2[i]= (p2)/mu2*(1-exp(-mu2*(time2[i]-start2[i])))+
                    ((a*p2)/mu2)*(cos(om)*time2[i]-exp(-mu2*(time2[i]-start2[i]))*cos(om)*start2[i])-
                     ((a*p2)/(om*mu2))*(1/(1+(1/(om^2*mu2^2))))*((1/mu2)*(sin(om)*time2[i]-exp(-mu2*(time2[i]-start2[i]))*sin(om)*start2[i])+
        ...
        biomass3[i]= (p3)/mu3*(1-exp(-mu3*(time3[i]-start3[i])))+
                    ((a*p3)/mu3)*(cos(om)*time3[i]-exp(-mu3*(time3[i]-start3[i]))*cos(om)*start3[i])-
                     ((a*p3)/(om*mu3))*(1/(1+(1/(om^2*mu3^2))))*((1/mu3)*(sin(om)*time3[i]-exp(-mu3*(time3[i]-start3[i]))*sin(om)*start3[i])+
        ...
        biomass4[i]= (p4)/mu4*(1-exp(-mu4*(time4[i]-start4[i])))+
                    ((a*p4)/mu4)*(cos(om)*time4[i]-exp(-mu4*(time4[i]-start4[i]))*cos(om)*start4[i])-
                     ((a*p4)/(om*mu4))*(1/(1+(1/(om^2*mu4^2))))*((1/mu4)*(sin(om)*time4[i]-exp(-mu4*(time4[i]-start4[i]))*sin(om)*start4[i])+
        ...
        biomass5[i]= (p5)/mu5*(1-exp(-mu5*(time5[i]-start5[i])))+
                    ((a*p5)/mu5)*(cos(om)*time5[i]-exp(-mu5*(time5[i]-start5[i]))*cos(om)*start5[i])-
                     ((a*p5)/(om*mu5))*(1/(1+(1/(om^2*mu5^2))))*((1/mu5)*(sin(om)*time5[i]-exp(-mu5*(time4[i]-start5[i]))*sin(om)*start5[i])+
        ...
        biomass6[i]= (p6)/mu6*(1-exp(-mu6*(time6[i]-start6[i])))+
                    ((a*p6)/mu6)*(cos(om)*time6[i]-exp(-mu6*(time6[i]-start6[i]))*cos(om)*start6[i])-
                     ((a*p6)/(om*mu6))*(1/(1+(1/(om^2*mu6^2))))*((1/mu6)*(sin(om)*time6[i]-exp(-mu6*(time6[i]-start6[i]))*sin(om)*start6[i])+
