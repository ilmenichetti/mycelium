...
fit <- stan(file = 'Myc_model_trees_variablemu_with_treatments1.stan', data = calib_data,  chains = 4, iter = iterations, cores=4, warmup = iterations-chain_length)
########################################################################################################
...
parms_names<-c(expression(paste(mu,"1")),
               ...
               expression(paste(mu,"6")),"z", expression(paste(P[0]," control/apatite")),
...
parms_list<-c("mu1","mu2","mu3","mu4", "mu5", "mu6",  "z", "p1", "p2", "p3", "p4", "p5", "p6")
  ...
  for( i in 1: dim(resampled_posteriors)[1]){
   ...
   #calculate the biomass
    ...
    for(k in 1:length(periods_vec1)){
        if(periods_vec1[k]==1){
        biomass_simulated_p1[i,k]<- (a1*p1)/mu1*(1-exp(-mu1*(calib_data$time1[k]-calib_data$start1[k])))+
          ((a1*a*p1)/mu1)*(cos(om)*calib_data$time1[k]-exp(-mu1*(calib_data$time1[k]-calib_data$start1[k]))*cos(om)*calib_data$start1[k])-
          ((a1*a*p1)/(om*mu1))*(1/(1+(1/(om^2*mu1^2))))*((1/mu1)*(sin(om)*calib_data$time1[k]-exp(-mu1*(calib_data$time1[k]-calib_data$start1[k]))*sin(om)*calib_data$start1[k])+
    ...
    a2=z*mean(tree2);
      for(k in 1:length(periods_vec1)){
        if(periods_vec2[k]==1){
        biomass_simulated_p2[i,k]<- (a2*p2)/mu2*(1-exp(-mu2*(calib_data$time2[k]-calib_data$start2[k])))+
        ((a2*a*p2)/mu2)*(cos(om)*calib_data$time2[k]-exp(-mu2*(calib_data$time2[k]-calib_data$start2[k]))*cos(om)*calib_data$start2[k])-
        ((a2*a*p2)/(om*mu2))*(1/(1+(1/(om^2*mu2^2))))*((1/mu2)*(sin(om)*calib_data$time2[k]-exp(-mu2*(calib_data$time2[k]-calib_data$start2[k]))*sin(om)*calib_data$start2[k])+
    ...
    for(k in 1:length(periods_vec1)){
        if(periods_vec3[k]==1){
        biomass_simulated_p3[i,k]<- (a3*p3)/mu3*(1-exp(-mu3*(calib_data$time3[k]-calib_data$start3[k])))+
        ((a3*a*p3)/mu3)*(cos(om)*calib_data$time3[k]-exp(-mu3*(calib_data$time3[k]-calib_data$start3[k]))*cos(om)*calib_data$start3[k])-
        ((a3*a*p3)/(om*mu3))*(1/(1+(1/(om^2*mu3^2))))*((1/mu3)*(sin(om)*calib_data$time3[k]-exp(-mu3*(calib_data$time3[k]-calib_data$start3[k]))*sin(om)*calib_data$start3[k])+
    ...
    for(k in 1:length(periods_vec1)){
        if(periods_vec4[k]==1){
        biomass_simulated_p4[i,k]<- (a4*p4)/mu4*(1-exp(-mu4*(calib_data$time4[k]-calib_data$start4[k])))+
        ((a4*a*p4)/mu4)*(cos(om)*calib_data$time4[k]-exp(-mu4*(calib_data$time4[k]-calib_data$start4[k]))*cos(om)*calib_data$start4[k])-
        ((a4*a*p4)/(om*mu4))*(1/(1+(1/(om^2*mu4^2))))*((1/mu4)*(sin(om)*calib_data$time4[k]-exp(-mu4*(calib_data$time4[k]-calib_data$start4[k]))*sin(om)*calib_data$start4[k])+
    ...
    for(k in 1:length(periods_vec1)){
      if(periods_vec5[k]==1){
        biomass_simulated_p5[i,k]<- (a5*p5)/mu5*(1-exp(-mu5*(calib_data$time5[k]-calib_data$start5[k])))+
        ((a5*a*p5)/mu5)*(cos(om)*calib_data$time5[k]-exp(-mu5*(calib_data$time5[k]-calib_data$start5[k]))*cos(om)*calib_data$start5[k])-
        ((a5*a*p5)/(om*mu5))*(1/(1+(1/(om^2*mu5^2))))*((1/mu5)*(sin(om)*calib_data$time5[k]-exp(-mu5*(calib_data$time5[k]-calib_data$start5[k]))*sin(om)*calib_data$start5[k])+
    ...
    for(k in 1:length(periods_vec1)){
      if(periods_vec6[k]==1){
        biomass_simulated_p6[i,k]<- (a6*p6)/mu6*(1-exp(-mu6*(calib_data$time6[k]-calib_data$start6[k])))+
        ((a6*a*p6)/mu6)*(cos(om)*calib_data$time6[k]-exp(-mu6*(calib_data$time6[k]-calib_data$start6[k]))*cos(om)*calib_data$start6[k])-
        ((a6*a*p6)/(om*mu6))*(1/(1+(1/(om^2*mu6^2))))*((1/mu6)*(sin(om)*calib_data$time6[k]-exp(-mu6*(calib_data$time6[k]-calib_data$start6[k]))*sin(om)*calib_data$start6[k])+
